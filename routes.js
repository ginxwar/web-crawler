...
//var fs = require("fs");
//var jquery = fs.readFileSync("./jquery-min.2.0.2.js").toString();
...
var JSONServices = {
  /*
  getPage: function() {
    return $.ajax("http://www.eap.ucop.edu/Participants/japan/Pages/1213_engineering_science_english_tohoku_year.aspx", {
      //dataType: "jsonp",
      success: function( data, textStatus, jqXHR ) {
        //nsole.log(data.length + " programs data received");
        //console.log(data);
        HTMLData.test1 = $(data);        
      }
    });
...
/*
// this gets the URLs first
 
var ss = [];

var selectorsss = $("div.country-details-cont div.mini-header:contains('2012-13') ~ ul li a[href*='1213']").each(function(program){
  var obj = {};
  obj["program"] = $(this).text();
...
var index = function(req, res){
  ...
  /*
  $.when( JSONServices.getPage()          
  ).done(function() {
    
    console.log(HTMLData.test1);
    res.end('test');
    
  });  //done
  */
  /*
  request('http://www.eap.ucop.edu/Participants/japan/Pages/1213_engineering_science_english_tohoku_year.aspx', function(error, response, body) {
...
//    });
    ...
    var nexttt = cheerioBody("table[summary='Program Calendar'] tbody strong")[1];
    
    var qwe = $(nexttt);
    
    //var test = _(cheerioBody("table[summary='Program Calendar'] tbody strong")[1])
    //.map(function(link){
//      return link;
//    });
    //var test = $("table[summary='Program Calendar'] tbody strong:eq(1)").text()
    console.log(qwe);
  ...
  */
  ...
  /*
  _(URLsMaster).each(function( url, index ){
  ...
  */
  ...
  //var domain = 'http://www.eap.ucop.edu/Participants/argentina/Pages/1213_intensive_spanish_fall.aspx';  
  //request(domain, processHTML);
  
  //var domain, indexEach;
  ...
  var qwe = function() {
    _(URLsSmall).each(function( url, index ) {
      
      var indexEach = index;
      var domain = url.url;
      var program = url.program;
      
      var goRequest = request(domain, function( err, res, html ) {
        var parsedHTML = $.load(html);    
        //console.log(domain);  //can access this due to execution context  :)
        ...
        parsedHTML("table[summary='Program Calendar'] strong").map(function(index, tag) {
          ...
          if (index === 1) {    //"second" bold statement in tr
            ...
            obj["index"] = indexEach;
            obj["program"] = program;
            obj["url"] = domain;    //can access this due to execution context  :)
  ...
  var qq = function(work, doneCallback) {
    work();
    doneCallback();
  };
  ...
  async.series([qwe], function(err, results){
    console.log(everything);
    console.log(err);
    console.log(results);
  });
